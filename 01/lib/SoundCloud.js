...
s.init = function( url, initComplate ){
    ...
    SC.get('/resolve', {url: url}, function(sound){
        ...
        //Make controller
        var tag = '<canvas id="SoundCloudController_debug"></canvas>\
        <div style="position: absolute;top: 0px;left: 0px; margin:10px;padding: 10px; color:#fff;background-color: #3a3a3a;">\
            <i class="fa fa-soundcloud" aria-hidden="true"></i>\
            <a style="color: #fff;font-family: sans-serif;font-size: 12px;margin-left: 10px;" href="' + sound.permalink_url + '" target="_blank" >' + sound.title + '</a>\
            <canvas style="margin-left: 10px;"id="SoundCloudController_equalizer"></canvas>\
        </div>';

        _menuControllerElement = document.createElement("div");
        _menuControllerElement.innerHTML = tag;
        _menuControllerElement.style.position = "absolute";
        _menuControllerElement.style.top = "0px";
        _menuControllerElement.style.left = "0px";
        // _menuControllerElement.style.width = "100%";

        document.body.appendChild(_menuControllerElement);
